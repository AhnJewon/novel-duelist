...
const CLAIM_PATTERNS = {
  ...
  silence:           /무효화|봉인|침묵/,
  maxHpGain:         /최대\s*체력/,
  // 🆕 DECISIONS #85 — 이제 진짜로 구현된 동작들. facts에 있으면 통과한다.
  destroy:           /파괴한다|파괴하고|파괴하며|제거한다|제거하고|제거하며/,
  searchDeck:        /서치|덱에서.{0,12}(찾|가져)/,
  summonToken:       /소환한다|소환하고|소환하며|토큰/,
  // 🐛 아래는 **엔진에 아예 없는** 동작이다. LLM이 TCG 관용구로 자주 지어낸다.
  //    facts에 나올 수 없으므로 언급되면 무조건 반려된다.
  //    (묘지가 없어 부활이, 소유권 이전이 없어 강탈이, 카드 교체가 없어 변신이 안 된다)
  _revive:           /부활|되살/,
  // ⚠️ 한국어 활용형에 주의: "훔치다 → 훔**친**다"처럼 어간이 바뀐다.
  //    /훔치/만 두면 "훔친다"를 놓친다 (실제로 놓쳤다).
  _steal:            /훔치|훔친|빼앗/,
  _transform:        /변신|둔갑/
};
