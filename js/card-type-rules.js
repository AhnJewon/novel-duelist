...
소환수는 콘셉트에 맞는 전투 스킬 **1개를 우선** 설계한다 (피해, 방어막, 회복, 연타, 상태이상 등).
기초 스탯보다 **스킬 효과가 카드의 정체성**이다 — 예산이 넘치면 시스템이 스탯을 먼저 깎아 효과를 살린다.
"skill": { "name": "스킬명", "damage": 12, ... } 에 효과 수치를 지정한다.
🃏 정말 효과가 없는 바닐라 소환수는 드물게만: "isVanilla": true 로 두고 수치를 비운다. 대신 "flavorText"에
세계관 한 줄(40자 이내)을 쓴다 — 효과처럼 읽히면 안 된다. 바닐라는 효과가 없는 만큼 스탯이 더 좋다.`,
