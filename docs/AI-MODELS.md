...
TIPO 본문 요청에서는 작가가 사실상 나오지 않는다(실측 0/3). 그래서 완성된 태그 뒤에 `artist:`를 마지막 줄로 두고
**작가만 따로 묻는 2차 호출**을 한다 — 매번 실제 Danbooru 작가 태그가 나온다(40~130ms). V4 계열에는 `artist:이름` 접두.
폴백: SLM 작가 → 설정의 작가 목록 → 화풍 태그. 통일감이 중요하면 `내가 지정`이 낫다. → [DECISIONS #48](DECISIONS.md), [#99](DECISIONS.md)
